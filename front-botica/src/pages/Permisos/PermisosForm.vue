<template>
    ...
    <q-form @submit.prevent="submit">
      <q-card-section class="q-pb-md">
        <q-input
          dense outlined v-model="form.permiso.name" :loading="form.validating"
        ...
        <q-input dense outlined v-model="form.permiso.description" label="Descripción" @change="form.validate(propPath + '.description')" :error="form.invalid(propPath + '.description')" :class="form.invalid(propPath + '.description') ? 'q-mb-sm' : ''">
          <template v-slot:prepend>
...
import formPermisos from './FormPermiso';
...
const form = ref(null)
if (props.id) {
  form.value = useForm('put', 'api/permisos/' + props.id, formPermisos)
} else {
  form.value = useForm('post', 'api/permisos', formPermisos)
}
