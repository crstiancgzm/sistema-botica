  ...
  <q-page>
    <div class="q-pa-md q-gutter-sm">
      <q-breadcrumbs>
        ...
        <q-breadcrumbs-el label="Lista de Proveedores" icon="mdi-key" />
      </q-breadcrumbs>
    </div>
    <q-separator />
    ...
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td auto-width>
            <q-btn v-if="userStore.hasPermission('admin-proveedores-editar')"
              size="sm"
              text-color="cyan-8"
              color="cyan-1"
              outline
              ...
              class="q-mr-xs"
            />
            <q-btn v-if="userStore.hasPermission('admin-proveedores-eliminar')"
              size="sm"
              text-color="red-13"
              color="red-1"
              outline
...
async function onRequest(props) {
  ...
  const { data, total = 0 } = await ProveedorService.getData({
...
async function editar(id) {
  ...
  edit.value = true;
  editId.value = id;
  const row = await ProveedorService.get(id);
  console.log(row);

  proveedoresformRef.value.form.setData({proveedor:row});
}
