<template>
  <q-dialog v-model="formSubcategorias" persistent>
    <CategoriaMalestarForm
      :title="title"
      :edit="edit"
      :id="editId"
      ref="subcategoriasformRef"
      prop-path="subcategoria"
      @save="save"
    ></CategoriaMalestarForm>
  </q-dialog>
  <q-page>
    <div class="q-pa-md q-gutter-sm">
      <q-breadcrumbs>
        ...
        <q-breadcrumbs-el label="Lista de Subcategorias" icon="mdi-key" />
      </q-breadcrumbs>
    </div>
    <q-separator />
...
const tableRef = ref();
const formSubcategorias = ref(false);
const subcategoriasformRef = ref();
const title = ref("");
...
async function onRequest(props) {
  ...
  const { data, total = 0 } = await CategoriaMalestarService.getData({
...
async function editar(id) {
  title.value = "Editar Subcategoria";
  formSubcategorias.value = true;
  edit.value = true;
  editId.value = id;
  const row = await CategoriaMalestarService.get(id);
  console.log(row);

  subcategoriasformRef.value.form.setData({subcategoria:row});
}
...
async function eliminar(id) {
  $q.dialog({
    ...
    await CategoriaMalestarService.delete(id);
    tableRef.value.requestServerInteraction();
    $q.notify({
      type: "positive",
